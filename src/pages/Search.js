...
class Search extends Component {
  constructor(props) {
    ...
    axios
      ...
      .then(response => {
        ...
        else this.setState({ currComp: this.noDataComp });
      })
      .catch(error => {
  ...
  componentWillReceiveProps(nextprops) {
    ...
    axios
      ...
      .catch(error => {
        if (this._ismounted) this.setState({ currComp: this.errorComp });
      });
  }
  noDataComp() {
    return (
    ...
    return (
      <div className="messagebox">
        <span className="messagebox-span">
          Пользователь {this.props.match.params.user} на lastfm не найден
        </span>
      </div>
    );
  }
      ...
      <div className="users-grid">
        {this.state.users.map(user => (
          <div
            ...
            className="users-grid-element"
          >
            <User
              user={{ name: user.username, url: user.image }}
    ...
    axios
      ...
      .then(response => {
        if (this._ismounted) {
          ...
          } else this.setState({ currComp: this.notFoundComp });
        }
      })
      .catch(error => {
        if (this._ismounted) this.setState({ currComp: this.errorComp });
  ...
  render() {
    return (
      <div>
        <SearchForm />
        <div className="content">{this.state.currComp()}</div>
        <div className="search-add-user">
          {this.props.match.params.hasOwnProperty("user") ? (
            <button onClick={this.addUser} className="search-add-user-button">
