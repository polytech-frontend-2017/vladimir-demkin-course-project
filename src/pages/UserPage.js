...
class UserPage extends Component {
  constructor(props) {
    ...
    this.state = {dataSet : [], currComp: this.defaultComp, user:{name:'', url:'#'}, userFound:false};
      axios
      ...
      .catch(error => {
         if(this._ismounted) this.setState({currComp:this.errorComp, userFound:false});
      });
...
componentWillUnmount() {
  ...
  noDataComp(){
    return(<div className='messagebox'>
         <span className='messagebox-span'>Нет данных для указанного периода</span>
         </div>);
  }
  loadedComp(){
    return(<div>
    ...
    return (
      ...
      <div className='content'>
        ...
        <div className='userpage-userholder'>  
          <User user={this.state.user} showDownloadLink={true} showStatLink={false}/>
        </div>  
         {this.state.userFound? <Controls user = {this.props.match.params.user}
