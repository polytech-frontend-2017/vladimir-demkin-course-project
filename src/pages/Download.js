...
class Download extends Component {
  constructor(props) {
    ...
    this.state = {dataSet : [], currComp: this.defaultComp, user:{name:'', url:'#'}, stop:true, max: 0, value:0,  userFound:false} ;
      axios
      ...
      .then(response => {
        if(response.data.length>0)
         this.setState({user : {name: response.data[0].username, url: response.data[0].image}, userFound:true});
        else this.setState({currComp:this.userNotFoundComp, userFound:false});
      })
      .catch(error => {
  ...
  loadingComp(){
    return(<div className='progressHolder'>
           <div>{this.state.max>0?this.state.value+'/'+this.state.max:''}
          </div>
           <progress max={this.state.max>0?this.state.max:'none'} value={this.state.value} style={{height:'100px', width:'600px'}}></progress>
          </div>);
  }
  loadedComp(){
    return(<div  className='messagebox'>
  ...
  apiDownComp(){
    return(<div className='messagebox'>
          <span className='messagebox-span'>Проблемы с соединением c api lastfm</span>
     </div>)
  }
 userNotFoundComp(){
     return(<div className='messagebox'>
          <span className='messagebox-span'>Пользователь не найден<br/>
      ...
      .then(response => {
        if(this._ismounted)
        {
          if(response.data.hasOwnProperty('max')){
            this.setState({max:response.data['max'], value:response.data['curr']});
          }
          else this.setState({currComp: this.apiDownComp, stop: true});
          if(!this.state.stop){
            if(parseInt(this.state.max)>parseInt(this.state.value)) setTimeout(this.load, 250 );
    ...
    return (
      ...
      <div className='content'>
        ...
        <div className='userpage-userholder'>  
          <User user={this.state.user} showDownloadLink={false} showStatLink={true}/>
        </div> 
        {this.state.userFound?
          <div className='big-icon-holder'>
          {this.state.stop?
            <div role='button'tabIndex='0'> <img src={startIcon} className='big-icon'  onClick={this.startLoading} /></div>
            :<div role='button' tabIndex='0'><img src={stopIcon} className='big-icon'  onClick={this.stopLoading} /></div>
          }
          </div>
          :null
        }
